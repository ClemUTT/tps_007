...
/*32*/ SELECT producteur.region, SUM(recolte.quantite), AVG(vin.degre) FROM vin, recolte, producteur WHERE producteur.id = recolte.producteur_id AND vin.id = recolte.vin_id AND vin.degre <= (SELECT AVG(vin.degre) FROM vin) GROUP BY producteur.region;
       /*Cette requete renvoie les quantités d'alcohol récoltées par région d'un degré inférieur ou égal à la moyenne de tous les vins récoltés*/
...
/*40*/ DROP TABLE vin;
    ...
    /*On ne peut pas supprimer une table qui contient des attributs faisant office de référence à des attributs d'autres tables de la base de donnnées.
...
/*Solution pour supprimer l'ensemble des relations :*/
       ALTER TABLE recolte DROP FOREIGN KEY recolte_ibfk_1;
       ALTER TABLE recolte DROP FOREIGN KEY recolte_ibfk_2;
       DROP TABLE vin, recolte, producteur;
